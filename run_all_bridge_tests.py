...
SRC_TEST_DIR = os.path.join(REPO_ROOT, 'src_test')
...
def main():
    ...
    for i, (ns, na) in enumerate(cases):
        ...
        if not run(
            ...
            continue
        # 3) Train (reads from data_autotest; may raise RuntimeError in algorithms.py)
        output_dir = os.path.join(REPO_ROOT, 'outputs', 'bridge_autotest', f'ns_{ns}_na_{na}')
        if not run(
            [
                VENV_PYTHON, 'train_bridge.py',
                '--num_states', str(ns),
                '--num_actions', str(na),
            ...
            cwd=SRC_TEST_DIR,
            description="train_bridge (e.g. RuntimeError: IAVI did not converge)",
        ):
            failed.append((ns, na, "train_bridge"))
            continue
        print(f"  OK ns={ns} na={na} -> outputs/bridge_autotest/ns_{ns}_na_{na}/")
