...
class BatchedIAVI:
    ...
    def __init__(self, K, num_states, num_actions, P, expert_policies, discount,
                 threshold=1e-3, alpha=0.1, max_iters=500, device='cuda'):
        self.K = K
        self.num_states = num_states
        self.num_actions = num_actions
        self.device = torch.device(device if torch.cuda.is_available() else 'cpu')
    ...
    def train(self):
        ...
        while e < self.max_iters:
            e += 1
            max_q = torch.max(self.q, dim=2).values  # (K, S)
            opt_nextv = self.discount * torch.einsum('san,kn->ksa', self.P, max_q)  # (K, S, A)

            eta = torch.log(self.expert_policy + self.epsilon) - opt_nextv
            eta_sum = eta.sum(dim=2, keepdim=True)
...
class PGIAVI:
    def __init__(self, num_latents, num_states, num_actions, P, train_trajs, test_trajs, discount,
                 ...
                 loss_threshold=1e-2, max_iterations=150, gate_mode='retrospective'):
    ...
    def intention_batch_mapping(self, e_loader, total_length):
        ...
        with torch.no_grad():
            for batch_states, batch_actions, batch_log_pi, batch_mask in e_loader:
                f_logits = self.target_intention_net(batch_states.to(self.device),
    ...
    def get_batch_log_pi(self, trajs, batched_iavi):
        if self.gate_mode == 'retrospective':
            agent_policies = batched_iavi.get_policies().to('cpu')
            agent_log_policies = torch.log(agent_policies + 1e-8)
        else:
            agent_log_policies = torch.log_softmax(
                batched_iavi.q.to('cpu'), dim=-1
            )
    ...
    def fit(self):
        ...
        for ds in ['train', 'test']:
            ...
            for i, seq_len in enumerate(mask_eval.sum(dim=1)):
                ...
                valid_i = mask_eval[i].cpu().numpy()
                gate_i = f_i.copy()
                gate_i[~valid_i] = 0.0
                responsibility_i = torch.exp(log_p_gamma[i, :, :]).cpu().numpy()
                responsibility_i[~valid_i] = 0.0
